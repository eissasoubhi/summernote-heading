...
Summernote Heading follows semantic versioning.
...
When a Heading release depends on a new `snb-components` API, publish and validate the shared runtime first. `summernote-bricks` should only be released afterward if it needs a dependency/range update.
